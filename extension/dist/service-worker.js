...
(() => {
  ...
  // ../lib/parser.ts
  function parseDirectoryEntry($2, element) {
    const header = $2(element).find(".directory-Entry_Header");
    const contacts = {};
    const fieldValues = header.find(".directory-Entry_FieldValue");
    header.find(".directory-Entry_FieldLabel").each((i, element2) => {
      if (element2.textContent) {
        contacts[element2.textContent.trim().toLowerCase()] = fieldValues[i].textContent?.trim();
      }
    });
    const entry = {
      name: header.find(".directory-Entry_Title")[0].textContent?.trim() ?? "",
      photo: header.find(".directory-Entry_PersonPhoto--full")[0].getAttribute("src") ?? "",
      contact: {
        email: "",
        ...contacts
      },
      households: [],
      grade: header.find(".directory-Entry_Tag")[0].innerHTML.replace("Grade", "").trim()
    };
    const householdDOM = $2(element).find(".directory-Entry_HouseholdSection");
    householdDOM.each((i, household) => {
      let address = void 0;
      const addressLink = $2(household).find(".directory-Entry_FieldTitle .directory-Entry_GoogleMapsLinkIcon")[0]?.parentElement;
      if (addressLink != null) {
        address = {
          text: addressLink?.parentElement?.textContent?.trim(),
          url: addressLink?.getAttribute("href")
        };
      }
      const parents = [];
      $2(household).find(".directory-Entry_FieldTitle--blue").each((i2, element2) => {
        const parentOuterElement = $2(element2.parentElement?.parentElement);
        const parentFieldValues = parentOuterElement.find(".directory-Entry_FieldValue");
        const output = {
          name: element2.textContent?.trim()
        };
        parentOuterElement.find(".directory-Entry_FieldLabel").each((i3, labelElement) => {
          if (labelElement.textContent) {
            output[labelElement.textContent.trim().toLowerCase()] = parentFieldValues[i3].textContent?.trim();
          }
        });
        parents.push(output);
      });
      entry.households.push({
        parents,
        address
      });
    });
  ...
  async function main() {
    const html = await getHTTP("https://portals.veracross.com/catlin/student/student/overview");
    console.log(html);
    const dom = $(html);
    const list = dom.find(".vx-list.course-list.future").get(0);
    const classes = [];
    ...
    await Promise.all(classes.map(async ({ name, id }) => {
      const html2 = await getHTTP(`https://classes.veracross.com/catlin/course/${id}/website/directory`);
      const dom2 = $(html2);
      console.error("Loading", name);
      dom2.find(".directory-Entry").each((i, element) => {
